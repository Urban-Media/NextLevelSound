...
class CacheRequest {
    ...
    public function __construct(\TheLion\OutoftheBox\Processor $_processor, $request = null) {
        if (empty($request)) {
            $request = $_REQUEST;
        }
        ...
        /* Set the max cache age to max expire age of temporarily links = 4 hours */
        if ($this->_max_cached_request_age >= (240 * 60)){
            $this->_max_cached_request_age = 239 * 60;
        }
    ...
    private function _set_user_identifier() {
        ...
        $user_specific_actions = array('addfolder', 'addfolder_role', 'delete', 'deletefiles_role', 'deletefolders_role', 'download_role', 'move', 'move_role', 'rename', 'renamefiles_role', 'renamefolders_role', 'upload', 'upload_role', 'view_role', 'view_user_folders_role');
        $permissions = array();
    ...
    public function clear_local_cache_for_shortcode() {

        $file_name = 'request_' . $this->get_processor()->get_listtoken();
