...
class Mega_Menu_Walker extends Walker_Nav_Menu {
    ...
    $loginExtraClass = ($item->title == "Login") ? 'login_nav nls_rounded_button_menu' : '';
    if ($item->title == "Login" || $item->title == "Logout") {
      $classes[] = $loginExtraClass;
    }
      ...
      if ($item->title == "Login" && is_user_logged_in()) {
        ...
        $item->url = wp_login_url(get_permalink(7));
      }
			...
			$item_output .= '</a>';
      if ($item->depth > 0 && $item->ID != 2164) {
        $item_output .= "<hr class='mega-sub-menu-hr'>";
      }
