<?php
/*
Plugin Name: NLS Recommended Plugins Widget
Plugin URI:
Description: Adds a recommended plugins widget to courses, sections and lessons
Version: 1.0
Author: Urban Media
 ...
 * Register the widget
 */
add_action('widgets_init', create_function('', 'return register_widget("NLS_Recommended_Plugins");'));

/**
 * Class NLS_Resources
 */
class NLS_Recommended_Plugins extends WP_Widget
{
	/** Basic Widget Settings */
	const WIDGET_NAME = "NLS Recommended Plugins";
	const WIDGET_DESCRIPTION = "Adds a recommended plugin widget for course/section/lesson sidebars";
    ...
    if (!is_plugin_active('advanced-custom-fields/acf.php')) {
      ...
      wp_die( __( 'NLS Recommended Plugins requires Advanced Custom Fields to be installed and activated.', 'NLS_Recommended_Plugins' ) );
    }
    ...
    $parentCourseID = $lesson->get_parent_course();
    $recommendedPluginsPageLink = get_field('recommended_plugins', $parentCourseID);
		extract($instance);
		?>
			<div class="section_sideblock nls_course_nav black_block">
        <div class="section_centre_circle">
          <img src="<?php echo get_template_directory_uri(); ?>/img/plug.png" class="circle_icon">
        </div>
        <h2 class="sideblock_title text-center white_text">
          Recommended Plugins
        </h2>
        <a href="<?php echo $recommendedPluginsPageLink; ?>" class="sideblock_link">
