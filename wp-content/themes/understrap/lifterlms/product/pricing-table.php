...
<?php if ( ! apply_filters( 'llms_product_pricing_table_enrollment_status', $is_enrolled ) && ( $purchaseable || $has_free ) ) : ?>
	...
	<section class="llms-access-plans cols-<?php echo $product->get_pricing_table_columns_count( $free_only ) ?>">
		...
		<?php foreach ( $product->get_access_plans( $free_only ) as $i => $plan ) : ?>
			...
			<div class="llms-access-plan<?php echo $plan->is_featured() ? ' featured' : ''; ?><?php echo $plan->is_on_sale() ? ' on-sale' : '' ?>" id="llms-access-plan-<?php echo $plan->get( 'id' ); ?>"  data-mh="accessplans">
				...
				<?php endif; ?>
					...
					<?php else : $plan->get_checkout_url(); ?>
						<a class="llms-button-action button nls_rounded_button nls_mark_complete" href="<?php echo $plan->get_checkout_url(); ?>"><?php echo $plan->get_enroll_text(); ?></a>
					<?php endif; ?>
