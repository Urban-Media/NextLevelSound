...
switch($type) {
  case "next":
    $cssClass = "nls_next_lesson";
  break;

  case "prev":
    $cssClass = "nls_prev_lesson";
  break;

  case "list":
  default:
    $cssClass = "nls_grey_button";
  break;
}
?>

<div class="llms-lesson-preview nls_course_nav <?php echo $cssClass; ?><?php echo $lesson->get_preview_classes(); ?>">
	<a class="llms-lesson-link<?php echo $restrictions['is_restricted'] ? ' llms-lesson-link-locked' : ''; ?>" href="<?php echo ( ! $restrictions['is_restricted'] ) ? get_permalink( $lesson->get( 'id' ) ) : '#llms-lesson-locked'; ?>"<?php echo $data_msg; ?>>
		...
		<section class="llms-main">
			<?php if ( 'lesson' === get_post_type( get_the_ID() ) ) : ?>
				<h6 class="llms-pre-text <?php echo $cssClass; ?>"><?php echo $pre_text; ?></h6>
			<?php endif; ?>
